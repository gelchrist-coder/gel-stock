...
`).join('') : 'No payments recorded'}
    ...
    createEmployee(event) {
        ...
        if (!validateGhanaPhone(phone)) {
            this.showNotification('Please enter a valid Ghana phone number', 'error');
            return;
        }
        ...
        try {
            ...
            if (!businessId) {
                this.showNotification('Cannot create employee: Business ID not found', 'error');
                return;
            }
            ...
            if (!businesses[businessId]) {
                this.showNotification('Business not found', 'error');
                return;
            }
            
            // Check if employee phone already exists
            if (businesses[businessId].employees && businesses[businessId].employees[formattedPhone]) {
                this.showNotification('Employee with this phone number already exists', 'error');
                return;
            }
            ...
            this.showNotification(`Employee "${name}" created successfully!`, 'success');
        } catch (error) {
    ...
    updateTransactionLog() {
        ...
        tbody.innerHTML = allTransactions.slice(0, 50).map(transaction => {
            ...
            const reference = transaction.reference || 'No Reference';
            const recordedBy = transaction.recorded_by || 'Unknown User';
            ...
            return `
                <tr>
                    ...
                    <td>${newStock}</td>
                    <td>${reference}</td>
                    <td><span class="recorded-by" title="${transaction.recorded_by_email || 'N/A'}">${recordedBy}</span></td>
                </tr>
            `;
        }).join('');
    }
